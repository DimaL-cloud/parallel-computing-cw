    ...
    private class Worker extends Thread {
        ...
        public void run() {
            ...
            while (true) {
                ...
                if (task != null) {
                    log.debug("Worker with id: {} executing task", id);
                    task.run();
                    log.debug("Worker with id: {} finished task", id);
                }
            }
        }

        private boolean shouldWaitForTask() {
            return (taskQueue.isEmpty() || status == ThreadPoolStatus.PAUSED)
                    && status != ThreadPoolStatus.SHUTDOWN;
        }
    }
